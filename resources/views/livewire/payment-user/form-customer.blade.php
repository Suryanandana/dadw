<section x-cloak class="grid grid-cols-1 gap-3 p-5 bg-gray-100 rounded xl:col-span-2" x-show="currentStep === 3"
    ...
    <div class="w-full col-span-2 p-3 bg-white rounded-lg">
        <form class="grid gap-4 mb-4 sm:grid-cols-2" wire:submit>
            <div>
            ...
            <div>
                ...
                <input wire:model.live.debounce.300ms='number' type="text" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg block w-full p-2.5 @error('phone') {{'outline-red-400 border-red-400'}} @enderror
            ...
            <label for="description" class="block -mb-2 text-sm font-medium text-gray-900 dark:text-white">Select
                Pax</label>
            <div class="grid grid-cols-3 gap-2 sm:col-span-2" x-data="{number: 0}">
                <div wire:click='setPax(1)'
                x-on:click="number = 1" x-bind:class="number === 1 ? 'bg-green-200' : 'bg-gray-200'"
                class="flex flex-col items-center justify-center p-5 rounded-lg cursor-pointer">
                ...
                <div wire:click='setPax(2)' 
                x-on:click="number = 2" x-bind:class="number === 2 ? 'bg-green-200' : 'bg-gray-200'"
                class="flex flex-col items-center justify-center p-5 rounded-lg cursor-pointer">
                    <svg class="w-6 h-6 text-gray-800 dark:text-white" aria-hidden="true"
                        xmlns="http://www.w3.org/2000/svg" width="24" height="24" fill="none" viewBox="0 0 24 24">
                ...
                <div wire:click='setPax(3)'
                x-on:click="number = 3" x-bind:class="number === 3 ? 'bg-green-200' : 'bg-gray-200'"
                class="flex flex-col items-center justify-center p-5 rounded-lg cursor-pointer">
                    <svg class="w-6 h-6 text-gray-800 dark:text-white" aria-hidden="true"
                        xmlns="http://www.w3.org/2000/svg" width="24" height="24" fill="none" viewBox="0 0 24 24">
                        <path stroke="currentColor" stroke-linecap="round" stroke-width="2"
